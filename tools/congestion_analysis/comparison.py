...
def load_metrics_config(path: Path, *, project_root: Path) -> dict[str, Any]:
    ...
    *,
    ...
    for heuristic in heuristics:
        ...
        for case_dir in sorted(path for path in heuristic_dir.iterdir() if path.is_dir()):
            if case_dir.name.startswith("_") or case_dir.name == "comparison":
                continue
...
def normalise_metric_frame(
    ...
    *,
    ...
    df.insert(4, "run_dir", str(run_dir))
    df.insert(5, "simulation_db_path", str(run_dir / "artifacts" / "db" / "simulation.db"))
...
def aggregate_case_heuristic(group: pd.DataFrame, metrics: list[str]) -> pd.Series:
    ...
    result: dict[str, Any] = {
        ...
        "run_dir": group["run_dir"].iloc[0] if "run_dir" in group.columns else "",
        "simulation_db_path": group["simulation_db_path"].iloc[0] if "simulation_db_path" in group.columns else "",
    }
