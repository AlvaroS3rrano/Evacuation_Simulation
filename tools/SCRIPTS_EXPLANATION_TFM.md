...
Generates four tables (CSV + one combined Markdown report):
...
Generates four figures (vector PDF for `\includegraphics` + PNG preview):
...
The color palette is fixed across all four charts (`none` in neutral gray, since it's the reference; `h1`, `h2`, `h3` in blue, teal, and orange respectively), so that the figures stay consistent with each other when included in the thesis.
...
| `build_thesis_result_tables.py` | Thesis tables (Tevac/D differences, synthesis, CV) |
| `build_thesis_result_figures.py` | Thesis figures (dispersion, means, deltas, trade-off) |
