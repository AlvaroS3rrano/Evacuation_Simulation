def compute_current_nodes(simulation_config, agent_group, frame) -> dict:
    """
    ...
    For each agent in the group, this function:
      ...
      - Finds the corresponding node from simulation_config.waypoints_ids.
    ...
    Returns:
        dict: A dictionary mapping each agent ID to its computed current node.
              If the agent or its node cannot be determined, its value will be None.
    """
    ...
    for agent_id in agent_group.agents:
        ...
        # Find the index of the next_node in the group's path.
        try:
            node_index = current_path.index(next_node)
        except ValueError:
            # next_node is not in the current path.
            current_nodes[agent_id] = None
            continue
        ...
        if node_index == 0:
            ...
            current_nodes[agent_id] = current_path[node_index - 1]
